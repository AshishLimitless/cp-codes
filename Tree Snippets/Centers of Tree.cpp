...
void dfs(vector<vll>&g,ll node,ll par,vll &dist,ll dis,vll &parv){
  dist[node]=dis;
  parv[node]=par;
  for(auto it:g[node]){
    if(it!=par){
      dfs(g,it,node,dist,dis+1,parv);
    }
  }
}
void madara(){
 ...
 // debug(path)
  ll dia=path.size()-1;
  if(dia%2==0){
  	ll center1=path[dia/2];
  	pri(center1)
  }
  else{
  	ll center1=path[dia/2];
  	ll center2=path[(dia+1)/2];
